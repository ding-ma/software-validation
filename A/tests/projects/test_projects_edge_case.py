...
def test_post_project_bool_string():
...
def test_delete_project_task_wrong_ids():
    r = requests.delete(url + "/100/tasks/100", headers=recv_json_headers)
    assert r.status_code == 404


def test_get_project_categories_wrong_id():
...
def test_post_project_categories_id_as_int():
    r = requests.post(url + "/100/categories", headers=recv_json_headers, json={"id": 100})
    assert r.status_code == 404


def test_delete_project_categories_wrong_ids():
