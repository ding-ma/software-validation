...
def test_get_todos_id():
    ...
    assert r.status_code == 200 and "todos" in todo and len(todo["todos"]) == 1 and int(
        todo["todos"][0]["id"]) == test_id
...
def test_get_todos_id_tasksof():
    ...
    r = requests.get(url_tod_id_tasksof % 1, headers=recv_json_headers)
    assert r.status_code == 200 and "projects" in r.json()


def test_head_todos_id_tasksof():
    r = requests.head(url_tod_id_tasksof % 1, headers=recv_json_headers)
    r_todos = requests.get(url_tod_id_tasksof % 1, headers=recv_json_headers)
    r_todos.headers.pop("date")
    r.headers.pop("date")
    # make sure HEAD does not return a message-body in the response and HTTP headers should be identical to GET 
    assert r.status_code == 200 and r.headers == r_todos.headers and not r.content
...
def test_post_todos_id_tasksof():
    ...
    r = requests.post(url_tod_id_tasksof % todo_id, data=json.dumps(project), headers=send_json_recv_json_headers)
...
def test_delete_todos_id_tasksof():
    ...
    r = requests.delete(url_tod_id_tasksof_delete % (int(original_todo["id"]), 1), headers=send_json_recv_json_headers)
...
def test_delete_todos_id_categories():
    ...
    r = requests.delete(url_todo_id_categories_delete % (int(original_todo["id"]), 1),
