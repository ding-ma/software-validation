...
def step_impl(context):
    for row in context.table:
        ...
        create_todo = requests.post(url_todo, data=json.dumps(
            {"title": str(row['task_title']), "description": str(row["task_description"]),
             "doneStatus": True if row["task_doneStatus"] == "True" else False}), headers=send_json_recv_json_headers)
...
def step_impl(context):
    for row in context.table:
        # create the todo
        create_todo = requests.post(url_todo, data=json.dumps(
            {"doneStatus": True if row["task_doneStatus"] == "True" else False, "title": str(row['task_title']),
             "description": str(row["task_description"])}), headers=send_json_recv_json_headers)
        todo_res = create_todo.json()
        todos = requests.get(url_todo).json()["todos"]
...
def step_impl(context):
    ...
    for row in context.table:
        # create the todo
        todo = {"title": str(row['task_title']), "description": str(row["task_description"]),
                "doneStatus": "true" if str(row["task_doneStatus"]) == "True" else "false"}
        result = [_["title"] == todo["title"] and _["description"] == todo["description"] and _["doneStatus"] == todo[
            "doneStatus"] for _ in todos]
        assert any(result)
